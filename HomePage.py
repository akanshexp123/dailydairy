...
def control_user():
    ...
    return render_template('Control_Page.html',name=user.name,imageUrl=source)
...
def test_user():
    ...
    try:
        answer_param = request.args.get('answer')
        if answer_param:
            # URL-decode and parse JSON
            answer = json.loads(unquote(answer_param))
    ...
    user = LeaderboardTable.query.filter_by(roll_number=roll_number).first()
    source = "../artifact/" + user.club.lower() + "_badge.png"
    return render_template('Qtest_Page.html', name=user.name, imageUrl=source, roll_number=roll_number, answer=answer)
...
def checkMeaningful(text,question):
    MISTRAL_API_KEY = 'BtaUKdhcy2MEbwpdDvSbX5xWrbcjXwsS'
...
def checkValidResponse(answers, previous, question):
    ...
    for answer in answers:
        validity = checkSanity(answer, previous) 
        if not validity:
            reply.append(False)
        else:
            reply.append(answer)
...
def submit_responses():
    ...
    goalReply = checkValidResponse(goalAnswer, prevQ1, Question[0])
    gratefulReply = checkValidResponse(gratefulAnswer, prevQ2, Question[1])
    smallwinReply = checkValidResponse(winAnswer, prevQ3, Question[2])

    answer = []
    for reply in goalReply:
        if reply is False:
            answer.append('')
            flashFag = True
    ...
    if flashFag:
        ...
        answer_json = quote(json.dumps(answer))
        return redirect(url_for('test_user', roll_number=roll_number, answer=answer_json))
